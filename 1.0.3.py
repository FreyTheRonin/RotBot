...
'**.rl** translates the last message in the channel (that is not a .rl command).\n        ' +
 '**.rl n** translates the last n messages in the channel, up to fifty.\n\n' +
...
'If anything is working incorrectly, DM @Frey#1251.')
            
hp = discord.Embed(title='How To Use', description=helptext, colour=0xFF0022)
...
async def on_message(message):
        if not message.author.bot:
                if message.channel.is_private:
                        ...
                        elif message.content.lower().startswith('.rl '):
                                rot = ''
                                n = 1;
